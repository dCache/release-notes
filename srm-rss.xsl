...
<!ENTITY base-uri     "http://www.dcache.org/downloads/1.9/srm" >
...
<!ENTITY all-feeds-filename "srm/rss-all-releases.xml">
]>
...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="series" mode="emit-feed">
    <exsl:document href="{concat('srm/rss-',@id,'-releases.xml')}">
      <xsl:apply-templates select="/" mode="emit-rss-contents">
  ...
  <xsl:template match="download-page" mode="emit-rss-contents">
	...
	<xsl:choose>
	  <xsl:when test="$id">
	    ...
	    <xsl:variable name="link-url" select="concat('&base-uri;#',$id)"/>
	  ...
	  <xsl:otherwise>
	    <xsl:variable name="title-text" select="'SRM releases'"/>
  ...
  <xsl:template match="release" mode="emit-rss-contents">
    <xsl:variable name="version" select="concat(../version-prefix,@version)"/>
    ...
    <xsl:variable name="notes-url"
		  select="concat('&base-uri;/', $notes-filename)"/>

    <xsl:variable name="release-anchor-url"
		  select="concat('&base-uri;/#', ../../@id, '-', @version)"/>
